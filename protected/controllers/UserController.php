...
class UserController extends Controller {
    ...
    public function actionKabinet() {  
        ...
        if ($_POST){ // перевод
            ...
            $log->from = $user_id;      
            $log->to = $_POST['Log']['to'];
            $log->count = $_POST['Log']['count'];                                   
            
            if ($log->validate() && $log->transferMoney()){            
                $log->save(false);
            }
            else{
                $values = $_POST['Log'];
        ...
        $usersList = CHtml::listData(User::model()->findAll(
                array(
                    'condition' => 'id!=:p',
                    'order' => 'name',
                    'params' => array(':p' => $user_id)
        )), 'id', 'name');       

        $this->render('kabinet', array(
